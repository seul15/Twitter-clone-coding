<template>
  <div class="fixed z-10 inset-0 overflow-y-auto" @click="$emit('close-modal')">
    <!-- $emit('') - '' 이벤트를 부모에게 방출 -->
    <div class="flex justify-center min-h-screen sm:pt-6 sm:pb-20 text-center sm:block sm:p-0">
      ...
      <div @click.stop class="inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        <!-- @click.stop 클릭이 적용되지 않음 -->
        <div class="border-b border-gray-100 p-2 flex items-center justify-between">
          <button @click="$emit('close-modal')" class="fas fa-times text-primary test-lg p-2 h-10 w-10 hover:bg-blue-50 rounded-full"></button>
          <!-- tweet button -->
        ...
        <div>
          ...
          <div class="flex px-4 pt-4 pb-3">
            ...
            <div class="ml-2 flex-1">
              <div class="flex space-x-2">
                <span class="font-bold text-sm">닉네임</span>
                <span class="text-gray text-sm">@이메일</span>
                <span class="text-gray text-sm">1시간</span>
              </div>
              <div class="text-sm">내용~</div>
              <div>
                <span class="text-primary text-sm">@이메일</span>
          ...
          <div class="flex px-4 pb-4">
            ...
            <div class="ml-2 flex-1 flex-col">
              <textarea v-model="tweetBody" rows="5" placeholder="내 답글을 트윗합니다" class="w-full text-lg font-bold focus:outline-none resize-none"></textarea>
