<template>
  <div class="flex flex-col items-center space-y-4 mt-10">
    ...
    <input v-model="id" type="text" class="rounded w-96 px-4 py-3 border border-gray-300 focus:ring-2 focus:border-primary focus:outline-none" placeholder="아이디"/>
    ...
    <button class="w-96 rounded-full bg-primary text-white py-4 hover:bg-dark" @click="onRegister">회원가입</button>
...
export default {
  setup() {
    ...
    const onRegister = () => {
      console.log(email.value, id.value, password.value)
    }
    return {email,id,password,loading,onRegister}
  }
}
</script>
